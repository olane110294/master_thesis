...
# for å generere tall fra weibull, bare kjør rWEI2(antall, parameter1, parameter2)
bud <- round(df_testkjoring$bud[4000:4500])/1000
deltakere <- round(rpois(501,3))
...
foo_inner <- function(i, theta1, theta2, lambda){
  function(N){
    ...
    ngittN = dbinom(deltakere[i], size = N, prob = pWEI2(r[i], theta1, theta2, lower.tail = FALSE), log = TRUE)
...
result_mle <- mle2(minuslogl = eqThree, start=list(theta1 = 0.5,
                                                  theta2 = 2,
                                                  lambda = 3),
                  method="L-BFGS-B", #lower=c(9.088e-09,1.001,2), nobs = length(bud), 
